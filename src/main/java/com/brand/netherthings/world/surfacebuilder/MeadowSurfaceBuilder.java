...
public class MeadowSurfaceBuilder extends NetherThingsSurfaceBuilder {

	public MeadowSurfaceBuilder(Function<Dynamic<?>, ? extends NetherThingsSurfaceConfig> function_1) {
		super(function_1);
	}

	public static final BlockState GRASSBLOCK;
	...
	public void generate(Random rand, Chunk chunk, Biome biome, int x, int z, int int_3, double double_1, BlockState blockState_1, BlockState blockState_2, int int_4, long seed, NetherThingsSurfaceConfig config) {
		...
		int run = -1;
		int offset = 100 + rand.nextInt(100);
		for (int y = 127; y >= 30; --y) {
			pos2.setY(y);
			++run;
			...
			if (state2 == NETHERRACK) {
				if (run < offset) {
					setBlockState(chunk, pos2, STONE);
				}
			} else {
				run = -1;
			}
